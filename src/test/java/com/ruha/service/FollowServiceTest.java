...
class FollowServiceTest {
    ...
    class GetFollowings {
        ...
        void 팔로잉_목록_조회_성공() {
            ...
            when(memberRepository.findByMemberIdAndIsDeletedFalse(currentMemberId))
                    .thenReturn(Optional.of(currentMember));
            when(followRepository.findByFollower(currentMember))
                    .thenReturn(List.of(follow1, follow2));

            // when
            List<FollowResponse> result = followService.getFollowings();

            // then
            assertThat(result).hasSize(2);
            assertThat(result.get(0).getFollowingNickname()).isEqualTo("following1");
            assertThat(result.get(1).getFollowingNickname()).isEqualTo("following2");
        }
    }
    ...
    class GetFollowers {
        ...
        void 팔로워_목록_조회_성공() {
            ...
            when(memberRepository.findByMemberIdAndIsDeletedFalse(currentMemberId))
                    .thenReturn(Optional.of(currentMember));
            when(followRepository.findByFollowing(currentMember))
                    .thenReturn(List.of(follow1, follow2));

            // when
            List<FollowResponse> result = followService.getFollowers();

            // then
            assertThat(result).hasSize(2);
            assertThat(result.get(0).getFollowerNickname()).isEqualTo("follower1");
            assertThat(result.get(1).getFollowerNickname()).isEqualTo("follower2");
        }
    }
